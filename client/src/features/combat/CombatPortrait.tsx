export function CombatPortrait({
  ...
  return (
    <div className="relative flex h-44 items-end justify-center overflow-hidden rounded-lg border border-slate-700/70 bg-slate-950/55">
      <div className="absolute inset-0 scene-backdrop opacity-80" />
      <div className="absolute bottom-0 h-16 w-full bg-gradient-to-t from-slate-950 to-transparent" />
      <div
        className={`relative mb-5 grid place-items-center ${
          isBoss ? "h-28 w-28" : "h-24 w-24"
        } ${hero ? "" : "scale-x-[-1]"}`}
        aria-label={name}
      >
